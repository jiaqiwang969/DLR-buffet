...
#include "velocityOutletNSCBCxFvPatchField.H"
...
template<class Type>
Foam::velocityOutletNSCBCxFvPatchField<Type>::velocityOutletNSCBCxFvPatchField
(
    const fvPatch& p,
...
Foam::velocityOutletNSCBCxFvPatchField<Type>::velocityOutletNSCBCxFvPatchField
(
    const velocityOutletNSCBCxFvPatchField& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
Foam::velocityOutletNSCBCxFvPatchField<Type>::velocityOutletNSCBCxFvPatchField
(
    const velocityOutletNSCBCxFvPatchField& ptpsf
)
:
...
Foam::velocityOutletNSCBCxFvPatchField<Type>::advectionSpeed() const
{
...
template<class Type>
Foam::tmp<Foam::scalarField>
Foam::velocityOutletNSCBCxFvPatchField<Type>::soundSpeed() const
{
...
template<class Type>
void Foam::velocityOutletNSCBCxFvPatchField<Type>::updateCoeffs()
{
	if (this->updated())
	{
		return;
	...
	if (lInf_ > 0)
	{
		...
		else if (ddtScheme == fv::backwardDdtScheme<scalar>::typeName || true)
		{
			Info<< "I am doing backward time scheme\n" << endl;
			// Calculate the field relaxation coefficient k (See A2.2.2)
			const scalarField K(etaAc_*(1.0-sqr(aP/cP))*cP/lInf_);
		        symmTensorField Lund(symmTensorField(aP.size()));
			scalarField deltaCoeffs=this->patch().deltaCoeffs();
...
template<class Type>
void Foam::velocityOutletNSCBCxFvPatchField<Type>::write(Ostream& os) const
{
